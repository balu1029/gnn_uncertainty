...
class OpenMMEnergyCalculation:
    ...
    def __init__(
        self, file: str = "datasets/files/alaninedipeptide/pdb/alaninedipeptide.pdb"
    ) -> None:
        ...
        integrator = LangevinIntegrator(
            self.temperature * kelvin, 1 / picosecond, 1 * femtoseconds
        )
    ...
    def set_positions(self, positions: np.array) -> None:
        ...
        positions = positions
        positions = [
            Vec3(position[0], position[1], position[2]) * nanometers
    ...
    def run_xyz_file(
        self, in_file: str, out_file: str, num_molecules: int = None
    ) -> np.array:
        """
        ...
        with open(out_file, "w") as file:
            for i in range(num_molecules):
                ...
                for j in range(len(molecules[i])):
                    file.write(
                        f"{atoms[j]} {molecules[i][j][0]} {molecules[i][j][1]} {molecules[i][j][2]}\n"
                    )
    ...
    def run_npy_file_parallel(
        self, in_file: str, out_file: str, num_molecules: int, num_threads: int
    ):
        """
    ...
    def generate_validation_from_numpy(
        self, in_file: str, out_file: str, num_molecules: int
    ) -> None:
        """
    ...
    def generate_in_distribution_from_numpy(
        self, in_file: str, out_file: str, num_molecules: int, valid: bool
    ) -> None:
        molecules = np.load(in_file)
        indices1 = [4, 6, 8, 14]
        indices2 = [6, 8, 14, 16]
        ...
        out_dist = molecules[np.delete(np.arange(len(molecules)), overlapping_indices)][
            -int(num_molecules / 2) :
        ]
        del molecules
        ...
        if not valid:
            self._generate_from_npy(
                in_dist, out_file_name + "_in_dist.xyz", num_molecules
            )
        else:
            self._generate_from_npy(
                in_dist,
                out_file_name + "_validation_in_dist.xyz",
                int(num_molecules / 2),
            )
            self._generate_from_npy(
                out_dist,
                out_file_name + "_validation_out_dist.xyz",
                int(num_molecules / 2),
            )

    def generate_validation_uniform(
        self, in_file: str, out_file: str, num_grids_per_dim: int, samples_per_grid: int
    ) -> None:
        molecules = np.load(in_file)
            ...
            for j in range(num_grids_per_dim):
                ...
                if num_samples > 0:
                    sampled_indices = np.random.choice(
                        overlapping_indices, size=num_samples, replace=False
                    )
                    if samples is None:
                        samples = molecules[sampled_indices]
                    else:
                        samples = np.concatenate(
                            (samples, molecules[sampled_indices]), axis=0
                        )
    ...
    def calc_forces(self, positions: np.array) -> np.array:
        ...
        for i in range(len(positions)):
            ...
            force = state.getForces(asNumpy=True).value_in_unit(
                kilojoules_per_mole / angstrom
            )
            forces.append(force)
        return np.array(forces)

    def _generate_from_npy(
        self, molecules: np.array, out_file: str, num_molecules: int
    ) -> None:
        energies = []
        atoms_ala = [
            "H",
        ...
        with open(out_file, "w") as file:
            for i in range(num_molecules):
                ...
                for j in range(len(molecules[i])):
                    file.write(
                        f"{atoms_ala[j]} {molecules[i][j][0]} {molecules[i][j][1]} {molecules[i][j][2]} {forces[j][0].value_in_unit(kilojoules_per_mole/angstrom)} {forces[j][1].value_in_unit(kilojoules_per_mole/angstrom)} {forces[j][2].value_in_unit(kilojoules_per_mole/angstrom)}\n"
                    )
