...
name = "swag5_20241113_193532"
cal = True

model_dir = f"gnn/models/{name}"
out_path = f"logs/{name}_reeval{"_cal4" if cal else ""}"
...
for i, model_name in enumerate(os.listdir(model_dir)):
    ...
    if uncertainty_method == "MVE":
        ...
        if cal:
            mve.calibrate_uncertainty(testloader_in, device=device, dtype=torch.float32, path=f"{out_path}/calibration{i}.pdf")
    ...
    if uncertainty_method == "SWAG":
        ...
        if cal:
            swag.calibrate_uncertainty(validationloader, device=device, dtype=torch.float32, path=f"{out_path}/calibration{i}.pdf")
    ...
    if uncertainty_method == "ENS":
        ...
        if cal:
            ens.calibrate_uncertainty(validationloader, device=device, dtype=torch.float32, path=f"{out_path}/calibration{i}.pdf")
    ...
    if uncertainty_method == "EVI":
        ...
        if cal:
            evi.calibrate_uncertainty(validationloader, device=device, dtype=torch.float32, path=f"{out_path}/calibration{i}.pdf")
