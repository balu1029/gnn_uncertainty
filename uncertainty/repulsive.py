...
class ModelEnsemble(BaseUncertainty):
    def __init__(self, base_model_class, num_models, *args, **kwargs):
        ...
        self.models = nn.ModuleList(
            [base_model_class(*args, **kwargs) for _ in range(num_models)]
        )
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            self.epoch_summary(
                epoch, use_wandb=use_wandb, lr=optimizer.param_groups[0]["lr"]
            )
    ...
    def train_epoch(
        ...
        for i, data in enumerate(train_loader):
            ...
            total_loss = (
                force_weight * stacked_loss_force + energy_weight * stacked_loss_energy
            )
    ...
    def predict(self, x, leave_out=None, *args, **kwargs):
        self.eval()
        energies, energy, forces, force = self.forward(
            x=x, leave_out=leave_out, *args, **kwargs
        )
    ...
    def forward(self, x, leave_out=None, *args, **kwargs):
        ...
        if leave_out is not None:
            ...
            energies = torch.stack(
                [model(x=x, *args, **kwargs) for model in self.models]
            )
        ...
        forces = torch.stack(
            [
                -torch.autograd.grad(
                    outputs=e, inputs=x, grad_outputs=grad_outputs, create_graph=True
                )[0]
                for e in energies
            ]
        )
