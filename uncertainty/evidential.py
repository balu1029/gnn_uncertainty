...
class EvidentialRegressionLoss(nn.Module):
    ...
    def NIG_Reg(self, y, gamma, v, alpha, beta):
        ...
        if self.kl:
            kl = self.KL_NIG(
                gamma, v, alpha, beta, gamma, self.omega, 1 + self.omega, beta
            )
            reg = error * kl
        else:
            evi = 2 * v + alpha
...
class EvidentialRegression(BaseUncertainty):
    ...
    def train_epoch(
        ...
        for i, data in enumerate(train_loader):
            ...
            energy, force, v, alpha, beta = self.forward(
                x=atom_positions,
                h0=nodes,
                edges=edges,
                edge_attr=None,
            ...
            total_loss = (
                force_weight * loss_force + energy_weight * evidential_loss_energy
            )
